...
def render_legend(elements):
    ...
    for ele in sorted(elements):
        color = ELEMENT_COLORS.get(ele, "#888888")
        legend_html += (
            f'<span style="display:inline-block; width:20px; height:20px; '
            f'background-color:{color}; border-radius:50%; margin-right:5px;"></span>{ele} &nbsp;&nbsp;'
        )
...
class VisualizeCIF(BaseModel):
    ...
    def run(self) -> bool:
        try:
            ...
            elements = {el.symbol for site in structure for el in site.species}
            # 用 py3Dmol 可视化
            ...
            for ele in elements:
                ...
                view.setStyle(
                    {"elem": ele},
                    {"sphere": {"color": color, "scale": 0.3}, "stick": {"color": color}}
                )
            view.zoomTo()
